...
const outputSizes = {
  ...
  emoji: { label: "絵文字画像", width: 180, height: 180, filenamePrefix: "emoji_", trimTransparent: true, padding: 10, zipName: "emoji-set.zip" }
};
...
for (const input of fileInputs) {
  input.addEventListener("change", () => {
    ...
    sourceFiles[group] = packageModes[packageMode].multiGroups.includes(group) ? files : files.slice(0, 1);
...
async function convertImage(file, index, size, transparentRgb, toleranceValueNumber, group) {
  ...
  const checks = buildChecks(name, outputCanvas.width, outputCanvas.height, size, hasTransparentPixels);
...
function getOutputFilename(group, index, size) {
  ...
  return `${size.filenamePrefix || ""}${String(index + 1).padStart(2, "0")}.png`;
}

function buildChecks(name, width, height, size, hasTransparentPixels) {
  const expectedName = size.fixedName
    ? name === size.fixedName
    : new RegExp(`^${escapeRegExp(size.filenamePrefix || "")}\\d{2}\\.png$`).test(name);
  return [
